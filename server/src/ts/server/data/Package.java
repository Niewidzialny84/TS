...
//Packing and unpacking Data object to byte array
public class Package {
    public static byte[] pack(Data data) {
        byte[] tab = new byte[14];
        int[] numbers = data.getNumbers();

        //byte moved from right to left to remain only as 2 bits and rest 0
        tab[0] = (byte)((data.getOperation().getByte()) << 6);
        //packing numbers by moving their bits backwards to be a size of byte
        //also an bitwise OR in the first section for getting whole byte of data
    ...
    public static Data unpack(byte[] data) {
        ...
        //bitwise AND needed beacause some errors occured and bits were placed wrong so its making sure that it is reading a correct one
        //getting the operation from byte array
        tmp.setOperation(Operation.getOperation((byte)((data[0] >>> 6) & 0x3)));
        //creating a number from byte parts
        ...
        //Reading a status and session from byte array
